...
import { z } from 'zod'
import { CreditCard, Truck, Shield, Check, Loader2, AlertCircle, Wallet, Smartphone } from 'lucide-react'
...
import { useCreateOrderMutation, useCreatePaymentIntentMutation, useVerifyPaymentMutation, useGetWalletQuery } from '../../../app/api'
...
const paymentMethods = [
  ...
  { id: 'UPI', label: 'UPI', desc: 'PhonePe, GPay, Paytm, BHIM', icon: Smartphone },
  { id: 'WALLET', label: 'TRADENEST Wallet', desc: 'Use your wallet balance', icon: Wallet },
]

const statusConfig = {
  RAZORPAY: { color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  STRIPE: { color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  UPI: { color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' },
  WALLET: { color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
}
...
export default function Checkout() {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<'RAZORPAY' | 'STRIPE' | 'UPI' | 'WALLET'>('RAZORPAY')
  ...
  // We need to get cart from API to calculate totals
  // For now, use local cart items with mock products
  const mockProducts: Record<string, { title: string; price: number; image: string }> = {
    // This will be populated from the cart API in real implementation
  }
  ...
  const handlePaymentMethodChange = (method: 'RAZORPAY' | 'STRIPE' | 'UPI' | 'WALLET') => {
  ...
  const handleRazorpayPayment = async (orderId: string, razorpayOrderId: string, razorpayKeyId: string, amount: number) => {
  ...
  const handleStripePayment = async (clientSecret: string) => {
    ...
    toast.success('Stripe payment initiated (simulated)')
    // Simulate successful payment
    return new Promise<{ paymentIntentId: string }>((resolve) => {
      setTimeout(() => {
        resolve({ paymentIntentId: 'pi_mock_' + Date.now() })
      }, 2000)
    })
  }

  const handleUPIPayment = async (upiIntent: string) => {
    // For UPI, we can try to open the UPI app
    // In production, this would integrate with Razorpay UPI or similar
    toast.success('Opening UPI app... (simulated)')
    ...
    return new Promise<{ upiRef: string }>((resolve) => {
      setTimeout(() => {
        resolve({ upiRef: 'upi_' + Date.now() })
      }, 3000)
    })
  }

  const handleWalletPayment = async (orderId: string, paymentId: string) => {
    // Wallet payment is processed on backend
  ...
  const onSubmit = async (data: CheckoutForm) => {
    ...
    try {
      ...
      const orderResult = await createOrder({
        shippingAddressId: 'addr-home', // In real app, this would be the address ID
      ...
      if (data.paymentMethod === 'RAZORPAY') {
        ...
        try {
          ...
          verificationResult = await verifyPayment({
            ...
            provider: 'RAZORPAY',
            providerRef: paymentResult.razorpayOrderId!,
        ...
        setStep(3)
        try {
          const stripeResponse = await handleStripePayment(paymentResult.stripeClientSecret!)
          
          verificationResult = await verifyPayment({
            paymentId: paymentResult.paymentId,
            provider: 'STRIPE',
            providerRef: paymentResult.stripeClientSecret!,
            stripePaymentIntentId: stripeResponse.paymentIntentId,
          }).unwrap()
        } catch (err: any) {
          throw new Error(err.message || 'Stripe payment failed')
        }
      } else if (data.paymentMethod === 'UPI') {
        setStep(3)
        try {
          const upiResponse = await handleUPIPayment(paymentResult.upiIntent!)
          
          verificationResult = await verifyPayment({
            paymentId: paymentResult.paymentId,
            provider: 'UPI',
            providerRef: paymentResult.upiIntent!,
          }).unwrap()
        } catch (err: any) {
        ...
        setStep(3)
        try {
          verificationResult = await handleWalletPayment(orderId, paymentResult.paymentId)
        } catch (err: any) {
      ...
      <div className="px-4 py-16">
        ...
        >
          ...
          <div className="space-y-3">
            <a href={`/orders/${paymentData?.orderId}`} className="btn btn-primary w-full">View Order</a>
    ...
    return (
      <div className="px-4 py-16">
        ...
        >
          ...
          <h1 className="mb-2 text-2xl font-bold text-secondary-900 dark:text-secondary-50">Processing Payment...</h1>
          <p className="mb-6 text-secondary-600 dark:text-secondary-400">Please wait while we verify your payment.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-8 lg:grid-cols-3">
            ...
            >
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  ...
                  <input {...register('fullName')} id="fullName" className="input" defaultValue={user ? `${user.firstName} ${user.lastName}` : ''} />
                ...
                <div>
                  ...
                  <input {...register('phone')} id="phone" type="tel" className="input" defaultValue={user?.phone || ''} />
            ...
            >
              ...
              <div className="grid gap-3">
                {paymentMethods.map((method) => {
                  ...
                  const insufficientFunds = isWallet && walletBalance < total
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <div className={clsx('flex h-10 w-10 items-center justify-center rounded-lg', statusConfig[method.id as keyof typeof statusConfig].color)}>
                      ...
                      <div className="flex-1">
                        ...
                        {isWallet && (
                          <div className="mt-2 flex items-center gap-2 text-sm">
                            <span className="text-secondary-600 dark:text-secondary-400">Balance:</span>
                            <span className="font-medium text-secondary-900 dark:text-secondary-50">₹{walletBalance.toLocaleString()}</span>
                            {insufficientFunds && (
              ...
              {watchedPaymentMethod === 'WALLET' && wallet && wallet.balance < total && (
                <div className="mt-4 p-4 rounded-lg bg-red-50 border border-red-200 dark:bg-red-900/20 dark:border-red-800">
                  <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
                    <AlertCircle className="h-5 w-5" />
                    <span>Insufficient wallet balance. Please add funds or choose another payment method.</span>
                  </div>
                </div>
              )}
            </motion.section>
          ...
          <div>
            ...
            >
              ...
              <button
                type="submit"
                disabled={processing || (watchedPaymentMethod === 'WALLET' && wallet && wallet.balance < total)}
